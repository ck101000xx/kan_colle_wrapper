...
class Fleet extends Observable implements IIdentifiable {
  ...
  void update(rawData) {
    ...
    updateShips(
        rawData["api_ship"].map((id) => _homeport.organization.ships[id]).toList());
  }
  ...
  void calculate() {
    ...
    totalViewRange =
        calcFleetViewRange(this, KanColleClient.current.settings.viewRangeCalcLogic);
    speed =
        ships.every((s) => s.info.speed == Speed.fast) ? Speed.fast : Speed.low;
  }
  ...
  void updateStatus() {
    ...
    isInShortSupply = ships.any(
        (s) => s.fuel.current < s.fuel.maximum || s.bull.current < s.bull.maximum);
    ...
    isReady = state == FleetState.homeport &&
  ...
  String toString() {
    return
        "ID = ${id}, Name = \"${name}\", Ships = ${ships.map((s) => "\"${s.info.name}\"").join(",")}";
  }

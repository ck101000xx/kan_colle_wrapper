...
class Quests extends Observable {
  ...
  Quests(KanColleProxy proxy) : _questPages = new List() {
    ...
    proxy["api_get_member_questlist"].asyncMap(
        serialize).where((x) => x != null).listen(update);
  }
  ...
  void update(questlist) {
    ...
    if (_questPages.length > questlist["api_page_count"]) {
      while (_questPages.length >
          questlist["api_page_count"]) _questPages.removeAt(_questPages.length - 1);
    } else if (_questPages.length < questlist["api_page_count"]) {
      // 小さいときは、キャッシュしたページ数と同じ数になるようにページを増やす
      while (_questPages.length <
          questlist["api_page_count"]) _questPages.add(null);
    }
    ...
    if (!questlist.hasProperty("api_list")) {
      ...
      questlist["api_list"].map((x) => new Quest(x)).forEach((x) => _questPages[page][x.id] =
          x);

      this.all = new UnmodifiableListView(
          new SplayTreeMap.fromIterable(
              _questPages.where((x) => x != null).expand((x) => x.values),
              key: (x) => x.id).values);

      var current = all.where(
          (x) =>
              x.state == QuestState.takeOn ||
                  x.state ==
                      QuestState.accomplished).toList()..sort((a, b) => a.id.compareTo(b.id));

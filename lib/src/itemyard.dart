...
class Itemyard extends Observable {
  ...
  Itemyard(KanColleProxy proxy) {
    ...
    tryParse(
        proxy["api_get_member_slot_item"]).listen((x) => updateSlotItems(x.data));
    tryParse(
        proxy["api_req_kousyou_createitem"]).listen((x) => createItem(x.data));
    tryParse(proxy["api_req_kousyou_destroyitem2"]).listen(destroyItem);
    tryParse(
        proxy["api_req_sortie_battleresult"]).listen((x) => dropShip(x.data));
    tryParse(
        proxy["api_get_member_useitem"]).listen((x) => updateUseItems(x.data));
  }
  ...
  void destroyItem(SvData data) {
    ...
    try {
      for (var x in data.request["api_slotitem_ids"].split(
          ',').map(int.parse)) {
        slotItems.remove(x);
      }
    } catch (ex) {
  ...
  void dropShip(source) {
    try {
      ...
      var target =
          KanColleClient.current.master.ships[source["api_get_ship"]["api_ship_id"]];
      if (target == null) return;

      _droppedItemsCount +=
          target.rawData["api_defeq"].where((x) => x != -1).length;
    } catch (ex) {

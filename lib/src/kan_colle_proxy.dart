...
class KanColleProxy {
  ...
  KanColleProxy() {
    ...
    _endpoints = new Map.fromIterables(
        endpoints.keys,
        endpoints.values.map((uri) =>
            apiSessionSource.where((session) =>
                session.request.url.path == uri)));
  }
  ...
  static final Map<String, Uri> endpoints =
      ...
      ].map(Uri.parse), key: _endpointName);

  static _endpointName(endpoint) =>
      "api_" +
      endpoint.pathSegments
          .sublist(1)
          .map((str) => str.replaceAll("api_", ""))
          .toSet()
          .join("_");

...
class Ship extends RawDataWrapper implements IIdentifiable {
  ...
  void update(rawData) {
    ...
    fuel =
        new LimitedValue(rawData["api_fuel"], info.rawData["api_fuel_max"], 0);
    bull =
        new LimitedValue(rawData["api_bull"], info.rawData["api_bull_max"], 0);

    if (rawData["api_kyouka"].length >= 5) {
      firepower =
          new ModernizableStatus(info.rawData["api_houg"], rawData["api_kyouka"][0]);
      torpedo =
          new ModernizableStatus(info.rawData["api_raig"], rawData["api_kyouka"][1]);
      aa =
          new ModernizableStatus(info.rawData["api_tyku"], rawData["api_kyouka"][2]);
      armer =
          new ModernizableStatus(info.rawData["api_souk"], rawData["api_kyouka"][3]);
      luck =
          new ModernizableStatus(info.rawData["api_luck"], rawData["api_kyouka"][4]);
    }

    slotItems = rawData["api_slot"].map(
        (id) => _homeport.itemyard.slotItems[id]).where((x) => x != null).toList();
  ...
  String toString() {
    return
        "ID = ${id}, Name = \"${info.name}\", ShipType = \"${info.shipType.name}\", Level = ${level}";
  }
}
